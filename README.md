...
worktree-gc collect codex-sessions --profile large-context
```
...
When Codex uses `--profile`, pass the same profile name so the collector layers
`$CODEX_HOME/NAME.config.toml` over the base config. If profile files exist but
no selection is supplied, configuration health is reported as incomplete.
...
correlation errors, separately measured unindexed rollout storage,
compression-marker age, and temporary artifacts. Missing or ambiguous files,
symlinks, incomplete bounded traversal, malformed or profile-ambiguous config,
