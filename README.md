...
When `[cleanup.pull_requests]` is enabled, `worktree-gc` batches exact commit
queries through the authenticated `gh` CLI. An exact-head open pull request
retains its worktree even after the ordinary age window. A clean, attached,
non-current exact-head worktree becomes routine-removal eligible after its pull
request has been merged for `merged_grace_days`. The manifest records the
repository, PR number and URL, PR head OID, merge time, observation time, and
evidence completeness. Incomplete GitHub or process-ownership evidence keeps
the worktree. Execution freshly revalidates Git identity, source status, PR
state, ownership, and protections before `git worktree remove`; it never uses
`--force` and never deletes the local branch. The equivalent one-off cleanup
flag is `--github-merged-pr-grace-days DAYS`, together with `--check-in-use`.
