...
fn plan_cleanup_with_protections_in_roots(
    ...
    let generated_decisions = generated_dirs
        ...
        .map(|dir| GeneratedDirDecision {
            ...
            worktree_in_use: dir.worktree_in_use,
...
fn revalidate_prepared_pressure_worktree(
    manifest: &CleanupManifest,
    worktree: &Path,
    exclusions: &[PathBuf],
    prepared: &PressureWorktreeSourceBaseline,
) -> Result<PressureWorktreeSourceBaseline> {
    revalidate_pressure_worktree(manifest, worktree)?;
    let current = pressure_worktree_source_baseline(worktree, exclusions)?;
    anyhow::ensure!(
        current == *prepared,
        "worktree head or grouped source baseline changed after pressure preparation"
    );
    Ok(current)
}
...
fn revalidate_prepared_pressure_epoch_worktrees(
    ...
    candidate_groups: &BTreeMap<(usize, PathBuf), Vec<PathBuf>>,
    prepared_sources: &mut BTreeMap<(usize, PathBuf), PressureWorktreeSourceBaseline>,
    metrics: &mut ExecutionMetrics,
    ...
    for key @ (index, worktree) in prepared_worktrees {
        let result = (|| {
            ...
            let exclusions = candidate_groups.get(key).with_context(|| {
                format!(
                    "pressure epoch has no prepared candidate group for {}",
                    worktree.display()
                )
            })?;
            ...
            revalidate_prepared_pressure_worktree(repository, worktree, exclusions, prepared)
        })();
        match result {
            Ok(current) => {
                prepared_sources.insert((*key).clone(), current);
...
fn current_prunable_worktree_paths(repo: &Path) -> Result<Vec<PathBuf>> {
    Ok(
        parse_worktree_list(&git_output(repo, ["worktree", "list", "--porcelain"])?)
            .into_iter()
            .filter(|worktree| worktree.prunable.is_some() || !worktree.path.exists())
...
fn scan_generated_dirs_for_worktree(
    ...
    let ownership_evidence_complete = policy.check_in_use
        && matches!(
    ...
    for candidate in candidates {
        ...
        let owner_free_pressure_mode = pressure_applies
            && policy
                .pressure
        ...
        // complete ownership evidence is the eligibility boundary.
        ...
        let (action, reason) = if candidate.action == GeneratedCandidateAction::ReportOnly {
        ...
        } else if owner_free_pressure_candidate && !ownership_evidence_complete {
            (
                GeneratedDirAction::Skip,
            ...
            if has_sweep_work {
                ...
                (
                    ...
                    format!("owned worktree with sweep work: {descriptions}"),
                )
            } else {
                (
                    GeneratedDirAction::Skip,
        ...
        } else if active && !owner_free_pressure {
            if has_sweep_work {
                let descriptions = sweeps
                    .iter()
                    .filter(|sweep| sweep.has_work())
            ...
            (
                ...
                if owner_free_pressure {
                    "owner-free ignored generated directory".to_string()
                } else {
            ...
            (
                GeneratedDirAction::Delete,
                if owner_free_pressure {
                    "owner-free untracked generated directory".to_string()
                } else {
        ...
        let cleanup_class = if action == GeneratedDirAction::Delete
            && pressure_applies
            && (owner_free_pressure || (effective_days < routine_days && routine_active))
        {
...
mod tests {
    ...
    #[test]
    fn dry_run_enables_metadata_prune_only_for_observed_prunable_paths() -> Result<()> {
        let (_temp, repo) = init_repo()?;
        let worktree = repo.with_file_name("missing-worktree");
        add_worktree(&repo, &worktree, "missing-worktree-branch")?;
        ...
        assert!(run.manifest.metadata_prune_enabled);
        assert!(run.manifest.worktrees.iter().any(|decision| {
            decision.metadata_prunable && decision.action == WorktreeAction::PruneMetadata
        }));
        Ok(())
    }
    ...
    #[test]
    fn routine_execution_fails_closed_when_prunable_metadata_changes_after_planning() -> Result<()>
    {
        let (_temp, repo) = init_repo()?;
        let planned = repo.with_file_name("missing-before-plan");
        add_worktree(&repo, &planned, "missing-before-plan-branch")?;
        ...
        let unplanned = repo.with_file_name("missing-after-plan");
        add_worktree(&repo, &unplanned, "missing-after-plan-branch")?;
        fs::remove_dir_all(&unplanned)?;
    ...
    fn bounded_pressure_worktree_revalidation_rejects_clean_head_drift() -> Result<()> {
        ...
        let error =
            revalidate_prepared_pressure_worktree(&run.manifest, &repo, &exclusions, &prepared)
                .expect_err("a clean checkout to another revision must invalidate the epoch");
        assert!(error.to_string().contains("head or source status changed"));
        Ok(())
    }
    ...
    #[test]
    fn owner_free_pressure_bypasses_generated_age_with_complete_evidence() -> Result<()> {
        let (_temp, repo) = init_repo()?;
        fs::create_dir_all(repo.join("node_modules/pkg"))?;
        ...
        assert!(decision.ownership_evidence_complete);
    ...
    #[test]
    fn owner_free_pressure_keeps_expired_generated_dirs_in_routine_pass() -> Result<()> {
        let (_temp, repo) = init_repo()?;
        let package = repo.join("node_modules/pkg");
        fs::create_dir_all(&package)?;
    ...
    #[test]
    fn owner_free_pressure_fails_closed_on_incomplete_ownership_evidence() -> Result<()> {
        let (_temp, repo) = init_repo()?;
        fs::create_dir_all(repo.join("node_modules/pkg"))?;
        fs::write(repo.join("node_modules/pkg/index.js"), "recent\n")?;
        let recent = unix_days_before_now(0);
        ...
        let run = plan_cleanup_with_protections(
            ...
            &[],
            Some(&OpenHandleSnapshot::Indeterminate),
        )?;
        ...
        assert!(decision
            .reason
            .contains("requires complete ownership evidence"));
        Ok(())
    }

    #[cfg(unix)]
    #[test]
    fn owner_free_pressure_rechecks_worktree_ownership_before_deletion() -> Result<()> {
        ...
        let _held = fs::File::open(repo.join("held-source.txt"))?;
        execute_cleanup_manifest(&run.manifest, ExecutionPass::PressureGenerated(3))?;
