...
const DOCKER_MANIFEST_VERSION: u64 = 1;
...
pub struct DockerPrunePlan {
    ...
    pub build_cache_private_reclaim_bytes: u64,
    pub build_cache_report_only_bytes: u64,
    pub image_private_reclaim_bytes: u64,
...
pub fn print_docker_collect(run: &DockerCollectRun) {
    ...
    println!(
        "build cache: {} candidates, {} private reclaim, {} candidate bytes",
        plan.build_cache_candidates.len(),
        format_bytes(plan.build_cache_private_reclaim_bytes),
    ...
    println!(
        ...
        format_bytes(plan.image_private_reclaim_bytes)
    );
    println!(
        "host: {} available at {}",
...
fn plan_docker_impl(
    ...
    let build_cache_candidate_bytes = build_cache_candidates
        .iter()
        .map(|candidate| candidate.size_bytes)
        .sum();
    let build_cache_private_reclaim_bytes = build_cache_candidates
        .iter()
        .map(|candidate| candidate.size_bytes)
        .sum();
    let build_cache_report_only_bytes = build_cache_report_only
        ...
        .map(|candidate| candidate.size_bytes)
        .sum();
    let image_private_reclaim_bytes = image_candidates
        .iter()
    ...
    Ok(DockerPrunePlan {
        ...
        build_cache_private_reclaim_bytes,
        build_cache_report_only_bytes,
        image_private_reclaim_bytes,
