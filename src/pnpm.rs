...
const PNPM_MANIFEST_VERSION: u64 = 1;
const MINUTES_PER_DAY: u64 = 24 * 60;
const DEFAULT_PNPM_SCAN_THREADS: usize = 4;
...
fn plan_pnpm_without_protection(
    ...
    plan.active_pnpm_processes = active_pnpm_processes()?;
    let (open_paths, open_handle_check_complete) = open_pnpm_paths(identity, &plan);
    plan.open_paths = open_paths;
    plan.open_handle_check_complete = open_handle_check_complete;
    plan.action = classify_plan(&plan);
    plan.reason = plan_reason(&plan);
    Ok(plan)
}
...
fn snapshot_content(
    ...
    *remaining = remaining.saturating_sub(visit.visited_entries);
    ...
    let mut complete = !visit.truncated;
    prefixes.sort();
...
fn classify_plan(plan: &PnpmPrunePlan) -> PnpmPruneAction {
    ...
    } else if !plan.active_pnpm_processes.is_empty() || !plan.open_paths.is_empty() {
...
#[cfg(unix)]
fn open_pnpm_paths(identity: &PnpmIdentity, _plan: &PnpmPrunePlan) -> (Vec<PathBuf>, bool) {
    // Supplying hundreds of store prefixes to `lsof +d` makes lsof rescan the
    // process table once per batch and can take minutes. One machine snapshot
    // is both more complete (it includes deeply open files) and substantially
    // cheaper; retain only names inside the canonical pnpm roots.
...
#[cfg(not(unix))]
fn open_pnpm_paths(_identity: &PnpmIdentity, _plan: &PnpmPrunePlan) -> (Vec<PathBuf>, bool) {
    (Vec::new(), false)
}

fn active_pnpm_processes() -> Result<Vec<String>> {
...
mod tests {
    ...
    fn lsof_snapshot_keeps_only_paths_inside_pnpm_roots() {
        ...
        let paths = parse_lsof_paths(
            b"p1\nfcwd\nn/cache/pnpm/store/v10/files/aa/hash\nf4\nn/cache/pnpm/cache/dlx/key/pkg\np2\nn/cache/other/file\n",
            &[&store, &cache],
        );
        assert_eq!(
            paths,
